...
@app.route('/')
def home():
    return "FinRobot Tıkır Tıkır Çalışıyor!"

def run_web_server():
    # Render ücretsiz tier'da bir port dinlemek zorundadır
            ...
            for update in yanit.get("result", []):
                ...
                if mesaj_metni.startswith("/ekle"):
                    ...
                    if len(parcalar) == 5:
                        ...
                        maliyet = float(parcalar[2])
                        hedef_yuzde = float(parcalar[3]) / 100
                        stop_yuzde = float(parcalar[4]) / 100
                        
                        portfoy[sembol] = {
                            "maliyet": maliyet,
                            "tp_fiyat": maliyet * (1 + hedef_yuzde),
                            "sl_fiyat": maliyet * (1 - stop_yuzde),
                            "tetiklendi_tp": False,
                            "tetiklendi_sl": False,
                            "son_fiyat": maliyet
                        }
                        ...
                        telegram_mesaj_gonder(f"✅ *{sembol}* listeye eklendi.")
                
                elif mesaj_metni == "/liste":
                    if not portfoy:
                        telegram_mesaj_gonder("📭 Liste boş.")
                    else:
                        liste_metni = "📋 *Canlı Durum:*\n\n"
                        for k, v in portfoy.items():
                            pb = "$" if not k.endswith(".IS") else "TL"
                            mal = v["maliyet"]
                            son = v.get("son_fiyat", mal)
                            kz = ((son - mal) / mal) * 100
                            durum = f"🟢 +%{kz:.2f}" if kz >= 0 else f"🔴 -%{abs(kz):.2f}"
                            liste_metni += f"📌 *{k}* ({durum})\nGüncel: {son:.2f} {pb} | Hedef: {v['tp_fiyat']:.2f} {pb}\n\n"
...
# --- SİSTEMİ ÇİFT KANALLI BAŞLATMA ---
if __name__ == "__main__":
    # Web sunucusunu arka planda başlat (Render kapamasın diye)
    t = threading.Thread(target=run_web_server)
    t.daemon = True
    t.start()
    
    # Ana bot motorunu çalıştır
    bot_ana_dongu()